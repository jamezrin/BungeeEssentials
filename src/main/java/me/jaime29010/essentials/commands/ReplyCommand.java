            ...
            if (player.hasPermission(main.getConfig().getString("permissions.message-send"))) {
                if (args.length >= 1) {
                    try (Jedis jedis = RedisHook.getJedisPool().getResource()) {
                        String string = jedis.hget("rce:lmp", player.getUniqueId().toString());
                        if (string != null) {
                            UUID uuid = UUID.fromString(string);
                            if (RedisHook.getRedisBungee().isPlayerOnline(uuid)) {
                                String message = PluginUtils.color(PluginUtils.joinArray(args));
                                ...
                                player.sendMessage(TextComponent.fromLegacyText(PluginUtils.color(
                                        ...
                                        .replace("%player%", RedisHook.getRedisBungee().getNameFromUuid(uuid))
                            ...
                            player.sendMessage(TextComponent.fromLegacyText(PluginUtils.color(
                                    main.getConfig().getString("messages.error-reply")
                            )));
                        }
                    }
                } else {
                    player.sendMessage(TextComponent.fromLegacyText(PluginUtils.color(
                            main.getConfig().getString("messages.reply-usage")
